...
AUKE_IDENTIFIER = 30
AUKE_SCENARIO = "BASE"
AUKE_MF_VARIABLES = ("mf_observed", "mf_prior", "mf_observed_minus_background", "mf_prior_minus_background")
...
def add_auke_enhancement_variables(ds):
    ...
    if "background" in ds:
        if "mf_prior" in ds:
            ds["mf_prior_minus_background"] = ds["mf_prior"] - ds["background"]
            ds["mf_prior_minus_background"].attrs.update(
                {
                    "long_name": "mf_prior minus background",
                    "units": ds["mf_prior"].attrs.get("units", ""),
                }
            )
...
: mole-fraction variables: ['mf_observed', 'mf_prior', 'mf_observed_minus_background', 'mf_prior_minus_background']
...
for species, ds in auke_base.items():
    ...
    fig, axes = plt.subplots(len(variables), 1, sharex=True, figsize=(13, 2.6 * len(variables)))
...
#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/07e78736c67974953e5dbf04f1da6645796df7c3.png]]
[[file:./.ob-jupyter/341a75726b41589617ac992abb6bcf5183f8a8a5.png]]
:END:
...
#+begin_src jupyter-python
for species, ds in auke_base.items():
    variables = [name for name in ("mf_observed", "mf_prior", "mf_observed_minus_background", "mf_prior_minus_background") if name in ds]
    fig, axes = plt.subplots(1, len(variables), figsize=(4.8 * len(variables), 4), sharey=False)
    axes = np.atleast_1d(axes)
...
#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/bbd36b5c31538bc8de47344f359174e78f89fcf0.png]]
[[file:./.ob-jupyter/cb4f777c9264a7ecd6146bc2335010342daff76a.png]]
:END:
...
Raw =mf_prior= includes the background term, while the current
=fp_dot_flux= output does not include a baseline. The most direct
comparison here is therefore expected to be =mf_prior_minus_background=.

#+begin_src jupyter-python
COMPARE_SPECIES = "co2"
COMPARE_VARIABLES = ("mf_prior_minus_background", "mf_observed_minus_background", "mf_prior", "mf_observed")
...
#+begin_src jupyter-python
COMPARE_SPECIES = "co2"
COMPARE_VARIABLES = ("mf_prior_minus_background", "mf_observed_minus_background")

fig, axes = plt.subplots(1, len(COMPARE_VARIABLES), figsize=(13, 4), sharey=False)
axes = np.atleast_1d(axes)
